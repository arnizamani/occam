...
showAxiom (x :->> y) = showExp x ++ " ->> " ++ showExp y
showAxiom (x :-> y) = showExp x ++ " -> " ++ showExp y
...
matchFuncArgs :: Bool -> Axiom -> WM -> [(WM,Maybe Axiom)]
--matchFuncArgs (HsFunBind (d@(HsMatch _ n patts (HsUnGuardedRhs rhs) _):xs)) func =
--    let (m,rhs') = matchesArg n patts rhs func
--    in if rhs' /= Nothing then (fromJust rhs',HsFunBind [d]) : (matchFuncArgs (HsFunBind xs) func) else matchFuncArgs (HsFunBind xs) func
--matchFuncArgs (HsFunBind (_:xs)) func = matchFuncArgs (HsFunBind xs) func
matchFuncArgs _ d@(lhs :->> rhs) func =
    let (m,rhs') = matchesArg lhs rhs func
    in if rhs' /= Nothing then [(fromJust rhs',Just d)] else [(func,Nothing)]
matchFuncArgs True d@(lhs :-> rhs) func =
    let (m,rhs') = matchesArg lhs rhs func
    in if rhs' /= Nothing then [(fromJust rhs',Just d)] else [(func,Nothing)]
matchFuncArgs False d@(lhs :-> rhs) func = [(func,Nothing)]

-- | Check if the function argument matches with the given expression
matchesArg :: Lhs -> Rhs -> WM -> (Bool, Maybe WM)
matchesArg (HsInfixApp e (HsQVarOp (UnQual n)) (HsInfixApp (HsVar (UnQual (HsIdent x))) (HsQVarOp (UnQual (HsSymbol "+"))) (HsLit (HsInt i))))
        exp
        arg@(HsInfixApp f (HsQVarOp (UnQual m)) (HsLit (HsInt j)))
    | n == m && matchExpExp e f
        = (True, Just $ replacePatExp (replacePatExp exp (HsVar (UnQual (HsIdent x))) (HsLit (HsInt (j - i)))) e f
          )

matchesArg (HsApp n (HsInfixApp x (HsQVarOp (UnQual (HsSymbol "+"))) (HsLit (HsInt i))))
        exp
        arg@(HsApp func (HsLit (HsInt j)))
    | n == func
        = (True, Just $ replacePatExp exp x (HsLit (HsInt (j - i))))
--matchesArg (HsApp (HsVar (UnQual n)) (HsCon (UnQual v))) exp arg@(HsCon (UnQual c))
--        = if c == v then (True, Just exp) else (False,Nothing)
matchesArg (HsApp (HsVar (UnQual n)) pat) exp (HsApp (HsVar (UnQual fname)) arg)
    | n == fname && matchExpExp pat arg && null (sameVarBinding $ nub $ expVarBinding pat arg)
        = (True, Just $ replacePatExp exp pat arg)

matchesArg (HsApp e@(HsApp _ _) lp) exp (HsApp f@(HsApp _ _) arg)
    = let (mach, rhs) = matchesArg e exp f
      in mach && rhs /= Nothing && matchExpExp lp arg
            ? (True, Just $ replacePatExp (fromJust rhs) lp arg)
            $ (False,Nothing)
matchesArg (HsApp e@(HsApp _ _) lp) exp (HsApp f@(HsCon _) arg)
    = let (mach, rhs) = matchesArg e exp f
      in mach && rhs /= Nothing && matchExpExp lp arg
            ? (True, Just $ replacePatExp (fromJust rhs) lp arg)
            $ (False,Nothing)

matchesArg (HsApp (HsApp (HsVar (UnQual n)) p1) p2) exp (HsInfixApp e1 (HsQVarOp (UnQual opname)) e2)
    | n == opname 
      && null (sameVarBinding $ nub (expVarBinding p1 e1) ++ (expVarBinding p2 e2) )
      && matchExpExp p1 e1 
      && matchExpExp p2 e2
        = (True, Just $ replacePatExp (replacePatExp exp p1 e1) p2 e2)
matchesArg (HsInfixApp p1 (HsQConOp (UnQual op)) p2) lhs (HsInfixApp e1 (HsQConOp (UnQual opname)) e2)
  | op == opname 
    && null (sameVarBinding $ nub (expVarBinding p1 e1) ++ (expVarBinding p2 e2) )
    && matchExpExp p1 e1 
    && matchExpExp p2 e2
        = (True, Just $ replacePatExp (replacePatExp lhs p1 e1) p2 e2)
matchesArg (HsInfixApp p1 (HsQVarOp (UnQual op)) p2) lhs (HsInfixApp e1 (HsQVarOp (UnQual opname)) e2)
  | op == opname
    && matchExpExp p1 e1
    && matchExpExp p2 e2
    && null (sameVarBinding $ nub $ (expVarBinding p1 e1) ++ (expVarBinding p2 e2) )    
        = (True, Just $ replacePatExp (replacePatExp lhs p1 e1) p2 e2)
matchesArg (HsList ps) exp (HsList es)
  | length es == length ps 
    && null (sameVarBinding $ concatMap (uncurry expVarBinding) $ zip ps es)
    && all (uncurry matchExpExp) (zip ps es) && not (null ps)
        = (True, Just (foldl (\rhs (p,e) -> replacePatExp exp p e) exp $ zip ps es))
--matchesArg (HsApp n p) rhs arg | matchPatExp (expToPat p) arg
--        = (True, Just rhs)
--matchesArg (HsApp n pat) rhs arg
--        = (True, Just $ replacePatExp rhs pat arg)
matchesArg lhs rhs func = --if matchExpExp lhs func then (True,Just rhs) else (False,Nothing)
        if lhs == func then (True,Just rhs) else (False,Nothing)
...
normalExp b e = case e of
    HsVar (UnQual (HsIdent f)) -> if b && f /= "x" && f /= "y" && f /= "z"
                                        then HsLit (HsString f)
                                        else e
    -- HsVar (UnQual (HsSymbol f)) -> if b then HsLit (HsString f) else e
    HsLit (HsInt i)        -> convertInt i
    HsLit (HsString s)     -> HsLit (HsString s) -- HsList (map (HsLit . HsChar) s)
    HsLit (HsStringPrim s) -> HsLit (HsString s) -- HsList (map (HsLit . HsChar) s)
    ...
    HsCon (UnQual (HsIdent (x:xs)))
                           -> if isLower x
                                then HsVar (UnQual $ HsIdent (x:xs))
                                else e
    HsInfixApp e1 op e2    -> HsInfixApp (normalExp b e1) op (normalExp b e2)
    HsApp (HsVar v) e2     -> HsApp (HsVar v) (normalExp b e2)
...
matchExpExp :: HsExp -> HsExp -> Bool
matchExpExp (HsCon x) (HsCon y) = x == y
matchExpExp (HsVar _) arg = True
matchExpExp (HsLit p) (HsLit e) = p == e
matchExpExp (HsNegApp p) (HsNegApp e) = matchExpExp p e
matchExpExp (HsInfixApp p1 (HsQVarOp (UnQual (HsSymbol "+"))) (HsLit (HsInt _))) (HsLit (HsInt _)) = True
matchExpExp (HsInfixApp p1 (HsQVarOp op1) p2) (HsInfixApp e1 (HsQVarOp op2) e2) | op1 == op2
        = matchExpExp p1 e1 && matchExpExp p2 e2
matchExpExp (HsInfixApp p1 (HsQConOp op1) p2) (HsInfixApp e1 (HsQConOp op2) e2) | op1 == op2
        = matchExpExp p1 e1 && matchExpExp p2 e2
matchExpExp (HsInfixApp p1 (HsQConOp (Special HsCons)) p2) (HsList (x:xs))
        = matchExpExp p1 x && matchExpExp p2 (HsList xs)
matchExpExp (HsInfixApp p1 (HsQConOp (Special HsCons)) p2) (HsLit (HsString (x:xs)))
        = matchExpExp p1 (HsLit (HsChar x)) && matchExpExp p2 (HsLit (HsString xs))
matchExpExp (HsTuple p) (HsTuple e) | length p == length e
        = all (uncurry matchExpExp) $ zip p e
matchExpExp (HsList p) (HsList  e) | length p == length e
        = all (uncurry matchExpExp) $ zip p e
matchExpExp (HsParen p) (HsParen e) = matchExpExp p e
matchExpExp (HsParen p) e           = matchExpExp p e
matchExpExp p            (HsParen e) = matchExpExp p e
matchExpExp (HsAsPat n2 p) (HsAsPat n1 e) | n1 == n2 = matchExpExp p e
matchExpExp HsWildCard _ = True
matchExpExp (HsIrrPat p) (HsIrrPat e) = matchExpExp p e
matchExpExp (HsCon p) (HsCon e) = p == e
matchExpExp (HsApp (HsCon p) p') (HsApp (HsCon e) e') | p == e = matchExpExp p' e'
matchExpExp (HsApp (HsVar p) p') (HsApp (HsVar e) e') | p == e = matchExpExp p' e'
...
expVarBinding :: HsExp -> HsExp -> [(HsName,HsExp)]
expVarBinding (HsVar (UnQual v)) arg = [(v,arg)]
...
patVarBinding :: HsPat -> HsExp -> [(HsName,HsExp)]
patVarBinding (HsPVar (HsIdent (v:vs))) arg | isUpper v =  []
patVarBinding (HsPVar v) arg = [(v,arg)]
patVarBinding (HsPLit p) _ = []
patVarBinding (HsPNeg p) (HsNegApp e) = patVarBinding p e
patVarBinding (HsPInfixApp p1 op1 p2) (HsInfixApp e1 (HsQVarOp op2) e2) | op1 == op2
        = patVarBinding p1 e1 ++ patVarBinding p2 e2
patVarBinding (HsPInfixApp p1 op1 p2) (HsInfixApp e1 (HsQConOp op2) e2) | op1 == op2
        = patVarBinding p1 e1 ++ patVarBinding p2 e2
patVarBinding (HsPInfixApp p1 (Special HsCons) p2) (HsList (x:xs))
        = patVarBinding p1 x ++ patVarBinding p2 (HsList xs)
patVarBinding (HsPInfixApp p1 (Special HsCons) p2) (HsLit (HsString (x:xs)))
        = patVarBinding p1 (HsLit (HsChar x)) ++ patVarBinding p2 (HsLit (HsString xs))
patVarBinding (HsPTuple p) (HsTuple e) | length p == length e
        = concatMap (uncurry patVarBinding) $ zip p e
patVarBinding (HsPList p) (HsList  e) | length p == length e
        = concatMap (uncurry patVarBinding) $ zip p e
patVarBinding (HsPParen p) (HsParen e) = patVarBinding p e
patVarBinding (HsPParen p) e           = patVarBinding p e
patVarBinding p            (HsParen e) = patVarBinding p e
patVarBinding (HsPAsPat n2 p) (HsAsPat n1 e) | n1 == n2 = patVarBinding p e
patVarBinding HsPWildCard HsWildCard = []
patVarBinding (HsPIrrPat p) (HsIrrPat e) = patVarBinding p e
patVarBinding (HsPApp p []) (HsCon e) = []
patVarBinding (HsPApp p [p']) (HsApp (HsCon e) e') | p == e = patVarBinding p' e'
patVarBinding (HsPApp p [p']) (HsApp (HsVar e) e') | p == e = patVarBinding p' e'
patVarBinding _ _ = []

-- | Returns multiple bindings for the same variable, if any exist
sameVarBinding :: [(HsName,HsExp)] -> [(HsName,HsExp)]
sameVarBinding [] = []
